...
// このパターンは2つの互換性のないクラスの一方が提供するAPIに適応させることで、
// それらのクラスを連動させるパターンです。
// 方法として、アプリケーションが期待するAPIをコンポーネントが提供するAPIにマッピングする。
// 以下にNewCoDirectoryをSearchToolクラスで使えるように適応させます。
